...
async function getGSCConnections(supabase: any, userId: string) {
  const isConnected = await isGoogleServiceConnected(supabase, userId, 'searchConsole');

  if (!isConnected) {
    return null;
  }

  const { data, error } = await supabase.from('gsc_connections').select('*').eq('user_id', userId);
...
async function getGTMConnections(supabase: any, userId: string) {
  const isConnected = await isGoogleServiceConnected(supabase, userId, 'tagManager');
  if (!isConnected) {
    return null;
  }

  const { data, error } = await supabase.from('gtm_connections').select('*').eq('user_id', userId);
...
async function getGAConnections(supabase: any, userId: string) {
  const isConnected = await isGoogleServiceConnected(supabase, userId, 'analytics');
  if (!isConnected) {
    return null;
  }

  const { data, error } = await supabase.from('ga_connections').select('*').eq('user_id', userId);
...
async function isGoogleServiceConnected(supabase: any, userId: string, service: string) {
  let tableName;
  switch (service) {
    case 'tagManager':
  ...
  if (data.expires_at) {
    ...
    if (expiryDate.getTime() < currentDate.getTime()) {
      try {
        ...
        if (deleteError) {
          console.error(`Error deleting expired token from ${tableName}:`, deleteError);
        }
      } catch (deleteException) {
        console.error(`Exception when deleting expired token from ${tableName}:`, deleteException);
      }
      return false;
    }
  }
