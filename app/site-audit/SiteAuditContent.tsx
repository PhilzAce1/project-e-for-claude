...
export default function SiteAuditContent({
  ...
  const refreshAudit = async () => {
    ...
    try {
      ...
      toast({
        title: 'Audit Started',
        description: 'Your site audit has been initiated. Results will be available shortly.',
      });
    } catch (error) {
      ...
      <div>
        <div className="md:flex md:items-center md:justify-between w-full overflow-hidden rounded-lg ring-1 bg-white ring-slate-900/10 p-8">
          <div className="flex items-center gap-x-3">
            <h1 className="font-serif text-2xl font-bold leading-7 text-gray-900 sm:truncate sm:text-3xl sm:tracking-tight">
              Site Audit:{' '}
              <a
                href={`https://${localSEOData?.domain}`}
                target="_blank"
            ...
            <button
              ...
              disabled={refreshing || localSEOData?.status === 'pending'}
              ...
              title={
                localSEOData?.status === 'pending' ? 'Audit in progress' : 'Refresh Site Audit'
              }
            >
              <svg
                ...
                strokeWidth={1.5}
                stroke="currentColor"
                className={`w-5 h-5 ${refreshing || localSEOData?.status === 'pending' ? 'animate-spin' : ''}`}
              >
                <path
                  strokeLinecap="round"
        ...
        <div className="flex flex-col items-center justify-center h-64">
          ...
          <p className="mt-4 text-gray-600">
            We're still working on your site audit. We'll email you when it's ready.
          </p>
        </div>
      </div>
    );
    ...
    switch (tabName) {
        ...
        return (
          <>
            <div className="border-b border-gray-200 pb-5">
              ...
              <h3 className="text-l leading-6 text-gray-900">
                Improvements required (In priority order)
              </h3>
            </div>
            <LighthouseAudits
      ...
      case 'Accessibility':
        return (
          <>
            <div className="border-b border-gray-200 pb-5">
              <h2 className="font-serif text-xl font-bold leading-6 text-gray-900">
                Accessibility
              </h2>
              <h3 className="text-l leading-6 text-gray-900">
                Improvements required (In priority order)
  ...
  const filteredChecks = Object.entries(page_metrics?.checks || ({} as Record<string, number>))
    .filter(
      ([key, value]: [string, unknown]) =>
        typeof value === 'number' && value > 0 && !excludedChecks.includes(key),
    )
        ...
        (payload) => {
          ...
          if (payload.new.status === 'completed' && payload.old.status === 'pending') {
            toast({
              ...
              description: 'Your site audit has been updated with fresh data.',
            });
          }
        },
      )
  ...
  }, [user?.id, seoCrawlData?.domain, supabase]);
  ...
  return (
    <div className="container mx-auto">
      <div className="md:flex md:items-center md:justify-between w-full overflow-hidden rounded-lg ring-1 bg-white ring-slate-900/10 p-8">
        <div className="flex items-center gap-x-3">
          ...
          <button
            ...
            disabled={refreshing || localSEOData?.status === 'pending'}
            className="flex items-center gap-x-2 rounded-full p-1 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 disabled:opacity-50"
            title={localSEOData?.status === 'pending' ? 'Audit in progress' : 'Refresh Site Audit'}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
      ...
      {/* Show loading state when audit is pending */}
      {localSEOData?.status === 'pending' ? (
        <div className="mt-8 text-center">
          ...
          <p className="mt-4 text-gray-600">Analyzing your site... This may take a few minutes.</p>
        </div>
      ) : (
        <>
          <dl className="mt-8 flex overflow-hidden  bg-white divide-x items-center rounded-2xl text-center ring-slate-900/10 ring-1">
            <div className=" p-8 flex-grow">
              <dd
                className={`order-first p-8 text-6xl inline-block font-semibold tracking-tight rounded-xl mb-4 ${getScoreBGColor(parseInt(onpage_score))} ${getScoreColor(parseInt(onpage_score))}`}
              >
                {parseInt(onpage_score)}
              </dd>
              <dt className="font-serif text-sm font-semibold  leading-6 text-gray-600 ">
                On Page SEO Score
              </dt>
            </div>
          ...
          <div className="mt-8 grid grid-cols-1 gap-0.5 overflow-hidden rounded-2xl  ring-slate-900/10 ring-1 sm:grid-cols-2 lg:grid-cols-2 ">
            <div className="flex flex-col bg-white p-8 relative pb-20">
              ...
              <p className="mt-2 text-sm text-gray-500">
                We've crawled {seoCrawlData?.total_pages || 'N/A'} pages and found{' '}
                {page_metrics?.non_indexable || 0} non-indexable pages for a total of{' '}
                {seoCrawlData?.total_pages || 'N/A'} pages discovered.
              </p>
              ...
              <ul className="mt-4">
                <li className="flex items-center gap-2">
                  ...
                  Successful:{' '}
                  <strong>
                    {seoCrawlData?.total_pages -
                      (page_metrics?.checks?.is_broken || 0) -
                      (page_metrics?.checks?.is_redirect || 0) || 'N/A'}
                  </strong>
                </li>
                <li className="flex items-center gap-2">
              ...
              <div className="absolute inset-x-0 bottom-0 bg-gray-50 px-4 py-4 sm:px-6 border-t text-sm">
                <a
                  href="/site-audit/pages"
                  className="font-medium text-orange-600 hover:text-orange-500 p-4 my-4 "
                >
                  View All Pages
                </a>
              </div>
            ...
            <div className="flex flex-col bg-white p-8 relative pb-20">
              ...
              <p className="mt-4 text-3xl font-bold">{totalIssues}</p>
              ...
              <ul className="divide-y divide-gray-200">
                {filteredChecks.slice(0, 4).map(([key, value]) => (
                  ...
                  >
                    <a
                      href={`/site-audit/issues/${key}`}
                      className="text-orange-600 hover:text-orange-500"
                    >
                      {String(value)} pages
                    </a>{' '}
                    {siteAuditDictionary[key as keyof typeof siteAuditDictionary] ||
                      `have an issue with ${key}`}
                    <a
                      href={`/site-audit/issues/${key}`}
                      className="text-orange-600 hover:text-orange-500 float-right"
                    >
                      View Details
                    </a>
                  </li>
          ...
          <div className="bg-white mt-8 grid grid-cols-1 gap-0.5 overflow-hidden rounded-2xl  ring-slate-900/10 ring-1 p-8">
            <h2 className="font-serif text-xl font-bold leading-6 text-gray-900">
              Site Speed - Powered by Google Lighthouse
            </h2>
          </div>
          {loading ? (
            <p>Loading audits...</p>
              ...
              <dl className="mt-8 border-b border-gray-200 grid grid-cols-1 gap-0.5 overflow-hidden rounded-2xl rounded-b-none text-center sm:grid-cols-2 lg:grid-cols-4 ring-slate-900/10 ring-1">
                {lighthouseStats.map((stat) => (
                  ...
                  >
                    ...
                    <dd
                      className={`rounded-lg p-4 order-first text-3xl font-semibold tracking-tight 
                                            ${
                                              activeTab === stat.name
                                                ? getActiveScoreColor(
