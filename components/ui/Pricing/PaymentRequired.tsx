  ...
  useEffect(() => {
    async function checkPaymentStatus() {
      try {
        ...
        subscription.filter(sub => (sub?.status === 'active' || sub?.status === 'trialing') && sub.prices.product_id === "prod_RJ6CHDZl8mv1QM")
        const hasActiveSubscription = subscription.filter(sub => sub?.status === 'active' || sub?.status === 'trialing');
