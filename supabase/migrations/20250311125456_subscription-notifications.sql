...
begin
  ...
  loop
    ...
    -- Calculate content percentage remaining
    content_percentage := content_remaining::float / subscription.total_content_allowed::float;
    
    -- Get last notification timestamp from metadata
    last_notification := (subscription.metadata->>'last_reminder_sent')::timestamp with time zone;
    
    -- Determine if we should notify based on days remaining and content percentage
    should_notify := false;
    
    -- NEW NOTIFICATION SCHEDULE:
    -- 14 days if more than half content left
    -- 7 days if more than quarter content left
    -- 4 days if any content left
    -- 1 day if any content left
    
    if content_remaining > 0 then
      if days_remaining = 14 and content_percentage > 0.5 then
        should_notify := true;
      elsif days_remaining = 7 and content_percentage > 0.25 then
        should_notify := true;
      elsif days_remaining = 4 and content_remaining > 0 then
        should_notify := true;
      elsif days_remaining = 1 and content_remaining > 0 then
        should_notify := true;
      end if;
      ...
      if should_notify and (
        ...
        date_trunc('day', last_notification) < date_trunc('day', now())
      ) then
        -- Send notification
        ...
        -- Update the last notification timestamp in metadata
