...
drop function if exists public.get_next_content_recommendation(uuid);

-- Create function that accepts business_id with a named parameter
...
BEGIN
    ...
    WITH competitor_opportunities AS (
        ...
        FROM get_keyword_opportunities(business_id_param) co
        WHERE NOT EXISTS (
...
-- Create legacy function that accepts user_id
create or replace function public.get_next_content_recommendation(user_id_param uuid)
returns table (
    keyword text,
    search_volume integer,
    opportunity_score double precision,
...
begin
    ...
    select id into business_id_var
    from business_information
    where user_id = user_id_param
    limit 1;

    -- Call the business_id version of the function
    return query
    select * from get_next_content_recommendation_by_business(business_id_var);
end;
$$;

-- Grant access to authenticated users
grant execute on function public.get_next_content_recommendation(uuid) to authenticated;
