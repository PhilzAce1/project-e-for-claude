...
CREATE POLICY "Enable read access for users with business access" ON competitors
    FOR SELECT USING (business_id IN (SELECT business_id FROM business_access_rights WHERE user_id = auth.uid()) OR user_id = auth.uid());
CREATE POLICY "Enable insert access for users with business access" ON competitors
    FOR INSERT WITH CHECK (business_id IN (SELECT business_id FROM business_access_rights WHERE user_id = auth.uid()) OR user_id = auth.uid());
CREATE POLICY "Enable update access for users with business access" ON competitors
    FOR UPDATE USING (business_id IN (SELECT business_id FROM business_access_rights WHERE user_id = auth.uid()) OR user_id = auth.uid());
CREATE POLICY "Enable delete access for users with business access" ON competitors
    FOR DELETE USING (business_id IN (SELECT business_id FROM business_access_rights WHERE user_id = auth.uid()) OR user_id = auth.uid());
...
CREATE POLICY "Enable read access for users with business access" ON business_analysis_answers
    FOR SELECT USING (analysis_id IN (SELECT id FROM business_analyses WHERE business_id IN (SELECT business_id FROM business_access_rights WHERE user_id = auth.uid()) OR user_id = auth.uid()));
CREATE POLICY "Enable insert access for users with business access" ON business_analysis_answers
    FOR INSERT WITH CHECK (analysis_id IN (SELECT id FROM business_analyses WHERE business_id IN (SELECT business_id FROM business_access_rights WHERE user_id = auth.uid()) OR user_id = auth.uid()));
CREATE POLICY "Enable update access for users with business access" ON business_analysis_answers
    FOR UPDATE USING (analysis_id IN (SELECT id FROM business_analyses WHERE business_id IN (SELECT business_id FROM business_access_rights WHERE user_id = auth.uid()) OR user_id = auth.uid()));
CREATE POLICY "Enable delete access for users with business access" ON business_analysis_answers
    FOR DELETE USING (analysis_id IN (SELECT id FROM business_analyses WHERE business_id IN (SELECT business_id FROM business_access_rights WHERE user_id = auth.uid()) OR user_id = auth.uid()));
